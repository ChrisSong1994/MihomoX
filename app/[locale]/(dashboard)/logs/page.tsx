    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            {t('subtitle') || (t.raw('title') === '系统日志' ? '查看内核实时输出日志' : 'View real-time kernel output logs')}
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            <ArrowDown className={`h-4 w-4 ${autoScroll ? 'translate-y-0.5' : ''}`} />
            {t('autoScroll')}
          </button>
          <button
            onClick={clearLogs}
      ...
      <div className="bg-slate-900 rounded-2xl shadow-xl overflow-hidden border border-slate-800">
        <div className="flex items-center gap-2 px-4 py-3 bg-slate-800 border-b border-slate-700">
          ...
          <span className="text-xs font-mono text-slate-400 ml-2">mihomo.log</span>
        </div>
        <div 
          ref={scrollRef}
          className="p-6 h-[calc(100vh-280px)] overflow-y-auto font-mono text-sm space-y-1 scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent selection:bg-indigo-500/30"
        >
          {logs.length === 0 ? (
            ...
            logs.map((log, i) => {
              ...
              return (
                <div key={i} className={`${textColor} break-all hover:bg-slate-800/50 rounded px-1 transition-colors leading-relaxed`}>
                  {log}
                </div>
              );
            })
